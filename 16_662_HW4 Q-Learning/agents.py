...
class QAgent():
    def __init__(self, environment, alpha=0.1, gamma=0.99, epsilon=0.1):
        ...
        for i in range(environment.height):
            for j in range(environment.width):
                state = (i, j)
                self.q_table[state] = {action: np.random.random() for action in environment.actions}
    ...
    def update(self, state, reward, next_state, action):
        ...
        max_next_q_value = max(self.q_table[next_state].values()) if next_state is not None else 0
