...
def CheckPointOverlap(pointsA, pointsB, axis):	
    ...
    Aproj = np.dot(pointsA, axis)
    Bproj = np.dot(pointsB, axis)

    if max(Aproj) >= min(Bproj) and max(Bproj) >= min(Aproj):
        overlap = True
    else:
        overlap = False
...
def CheckBoxBoxCollision(pointsA, axesA, pointsB, axesB):
    ...
    for axisA in axesA:
        if  CheckPointOverlap(pointsA, pointsB, axisA) is False:
            return False
    
    for axisB in axesB:
        if CheckPointOverlap(pointsA, pointsB, axisB) is False:
            return False
    ...
    for axisA in axesA:
        ...
        for axisB in axesB:
            ...
            if CheckPointOverlap(pointsA, pointsB, axis) is False:
                return False
