...
def InitializePreconditionsAndEffects(nrObjects, nrPredicates, Predicates, Objects):
    ...
    Effect[0][1]= -2. #Robot not in kitchen
    ...
    for j in [1,2]:
        ...
        Precond[4][1]= 1   # Knife in kitchen
...
while len(pq)>0:
    ...
    if CheckCondition(state, GoalState):
        FoundPath=True
        break

    for i in range(len(ActionPre)):

        if CheckCondition(state,ActionPre[i]):

            next_state = ComputeNextState(state, ActionEff[i])

            # Check if state is visited
            if not CheckVisited(next_state, vertices):
                
                # Djikstra's without heuristic
                new_cost = cost2come[x] +1  

                # A star with heuristic
                heuristic_cost = new_cost+ Heuristic(next_state, GoalIndicesOneStep, GoalIndicesTwoStep)  
                ...
                # a star uncomment
                heapq.heappush(pq, (heuristic_cost, len(vertices)-1)) 
                # djiksttras uncomment
                #heapq.heappush(pq, (new_cost, len(vertices)-1)) 
